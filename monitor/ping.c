...
int main(int argc,char **argv)
{
    ...
    if (setsockopt(icmp_sock,SOL_IP,IP_RECVERR,(char *)&hold,sizeof(hold)))
        fprintf(stderr,"WARNING: your kernel is veeerrry old.No problems.\n");
        ...
        switch (icp->type)
        {
            ...
            case ICMP_PARAMETERPROB:
            {
                ...
                if (error_pkt)
                {
                    ...
                    {
                        static int once;
                        // Sigh,IP_RECVERR for raw socket was broken until 2.4.9.
                        // SO, we ignore the first error and warn on the second.
                        if (once++ == 1)
                            fprintf(stderr,"\rWARNING: kernel is not very fresh, upgrade is recommanded.\n");
                        if (once == 1)
                            return 0;
                    }
                }
...
int parsetos(char *str)
{
    ...
    if (tos > TOS_MAX)
    {
        fprintf(stderr,"ping: the decimal value of TOS bits mut be 0-254 (or zero)\n");
        exit(2);
    }
    return (tos);
}
...
u_short in_cksum(const u_short *addr,register int len,u_short csum)
{
    ...
    // we add sequential 16 bit words to it, adn at the end, fold
...
// and optionally with a hostname
