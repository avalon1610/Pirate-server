...
long nreceived;         // # of packets we get back
...
// Protocol independent setup and paremeter checks
void setup(int icmp_sock)
{
    ...
    if (options & F_MARK)
    {
        ...
        if (ret == -1)
            // we probably don't wanna exit since old kernels don't support mark.
...
void main_loop(int icmp_sock,__u8 *packet,int packlen)
{
    ...
    for (;;)
    {
        ...
        // "next" is time to send next , if positive.
        ...
        if ((options & (F_ADAPTIVE|F_FLOOD_POLL)) || next < SCHINT(interval))
        {
            ...
            {
                // Very short timeout... SO, if we wait for something, we sleep
                // for MININTERVAL. Otherwise,spin!
                if (recv_expected)
                {
                    next = MININTERVAL;
                }
                ...
                {
                    next = 0;
                    // When spinning, no reason to poll. Use nonblocking recvmsg() instead.
        ...
        for (;;)
        {
            ...
            // someone runs another ping on this host.
            if (not_ours)
                install_filter();
...
int gather_statistics(__u8 *icmph,int icmplen,int cc,__u16 seq,int hops,
...
// will be added on by the kernl. The ID field is our UNIX process ID,
...
int pinger(void)
{
    ...
    // Have we already send enough? If we have, return an arbitrary positive value.
...
void common_options(int ch)
{
    switch(ch)
    {
        ...
        case 'f':
            ...
            // fallthrough to numeric - avoid gethostbyname during flood
        case 'n':
            options |= F_NUMERIC;
            break;
        case 'p':
...
void status(void)
{
    ...
    if (nreceived && timing)
    {
        ...
        fprintf(stderr,", min/avg/ewma/max = %ld.%03ld/%lu.%03ld/%d.%03ld/%ld.%03ld ms",
                (long)tmin/1000,(long)tmin%1000,
                tavg/1000,tavg%1000,
                rtt/8000,(rtt/8)%1000,
                (long)tmax/1000,(long)tmax%1000);
